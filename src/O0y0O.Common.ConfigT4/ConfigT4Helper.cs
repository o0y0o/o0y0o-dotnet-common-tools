...
namespace O0y0O.Common.ConfigT4
{
    public static class ConfigT4Helper
    {
        ...
        private static string GenerateConfigCode(string namespaceName, ConfigT4Option[] options, JsonObject[] configs)
        {
            ...
            code.AppendLine("using Microsoft.Extensions.Configuration;");
            code.AppendLine();
            code.AppendLine($"namespace {namespaceName};");
            code.AppendLine();
            code.AppendLine("public static class Config");
            code.AppendLine("{");
            code.AppendLine("    private static IConfiguration? _instance;");
            code.AppendLine("    private static IConfiguration Instance =>");
            code.AppendLine("        _instance ?? throw new InvalidOperationException(\"Configuration has not been initialized.\");");
            code.AppendLine();
            code.AppendLine("    public static void Initialize(IConfiguration config)");
            code.AppendLine("    {");
            code.AppendLine("        _instance = config;");
            code.AppendLine("    }");
            code.AppendLine();
            code.AppendLine("    private static T GetValue<T>(string key, T defaultValue = default(T)) =>");
            code.AppendLine("        Instance.GetValue<T>(key) ?? defaultValue;");
            code.AppendLine();
            code.AppendLine("    private static T GetSection<T>(string key, T defaultValue = default(T)) =>");
            code.AppendLine("        Instance.GetSection(key).Get<T>() ?? defaultValue;");
            code.AppendLine();
        ...
        private static string GetValueType(JsonValue value)
        {
            switch (value.GetValueKind())
            {
                ...
                case JsonValueKind.False:
                    return "bool";
                case JsonValueKind.Number:
                    return value.TryGetValue(out int _) ? "int" : value.TryGetValue(out long _) ? "long" : "decimal";
                case JsonValueKind.String:
                    return "string";
                default:
                    throw new ArgumentException("Unexpected value kind.");
            }
        }
    }
}